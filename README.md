...
where $r$ and $k$ are the radial coordinates in the aperture and sensor planes, $R$ is the aperture radius, and $J_0$ is the zeroth-order Bessel function of the first kind. Under defocus and spherical aberrations, the pupil phase becomes:

$$
P(r) = \exp\left(j 2 C_d  r^2 / R^2\right) \exp\left(j C_s  r^4 / R^4\right)
$$

with $C_d$ and $C_s$ as the defocus coefficient and spherical aberration coefficient. This integral has no closed form in general because of $J_0$ — so the key idea behind this paper was to replace $J_0$ with a piecewise approximation:
...
4. **Optimized quadratic coefficients (`alpha`)**: the quadratic-fit coefficient for each ring depends on `(Cd, Cs)` and is not itself closed-form; it was found by sweeping alpha per ring for a grid of `(Cd, Cs)` pairs. The results of that sweep live in `data/alpha_coefficients.csv` and are loaded via `src/alpha_table.py`.
